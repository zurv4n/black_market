...
-- * EVENTS * --
RegisterServerEvent('black_market:Client:sellItems', function(dataItems, storeId)
...
RegisterServerEvent('black_market:Client:buyItems', function(dataItems, storeId)
    local _source = source
    local User = VORPcore.getUser(_source)
...
-- * CALLBACKS * --
VORPcore.addRpcCallback('black_market:callback:getShopStock', function(source, cb, args)
...
VORPcore.addRpcCallback('black_market:callback:ShopStock', function(source, cb, args)
    local data = {
...
VORPcore.addRpcCallback('black_market:callback:getPlayerJob', function(source, cb, args)
    local _source = source
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    --Determin Which Black Market Stores to Open
    store_one_int = math.random(1,4)
    store_two_int = math.random(5,8)

    if store_one_int == 1 then
        store_one = 'One'
    elseif store_one_int == 2 then
        store_one = 'Two'
    elseif store_one_int == 3 then
        store_one = 'Three'
    elseif store_one_int == 4 then
        store_one = 'Four'
    end
    if store_two_int == 5 then
        store_two = 'Five'
    elseif store_two_int == 6 then
        store_two = 'Six'
    elseif store_two_int == 7 then
        store_two = 'Seven'
    elseif store_two_int == 8 then
        store_two = 'Eight'
    end

    for storeId, storeConfig in pairs(Config.Stores) do
        if storeConfig.RandomPrices then
            for ind, storeTypeSpecific in pairs(storeConfig.storeType) do
                if storeTypeSpecific.Type == 'sell' then
                    for index, storeItem in ipairs(Config.SellItems[storeId]) do
                        Config.SellItems[storeId][index].sellprice = storeItem.randomprice
                    end
                elseif storeTypeSpecific.Type == 'buy' then
...
RegisterServerEvent('black_market:GetRefreshedPrices', function()
    local _source = source
    TriggerClientEvent('black_market:RefreshStorePrices', _source, Config.SellItems, Config.BuyItems)
    ...
    TriggerClientEvent("vorp_storesNew:Server:tableOfJobs", _source, Jobs)
end)

--Zurv4n added
RegisterServerEvent('black_market:GetOpenBlackMarket', function()
    local _source = source
    openMarkets = {store_one,store_two}
    TriggerClientEvent('black_market:openMarkets', _source, openMarkets)
    --print("Server is supposed to send:")
    --print(store_one)
    --print(store_two)
    --return openMarkets
end)
...
RegisterNetEvent("vorp:SelectedCharacter", function(source, character)
    ...
    TriggerClientEvent("black_market:Server:tableOfJobs", _source, Jobs)
end)
