...
-- Zurv4n added
RegisterNetEvent("black_market:openMarkets", function(openMarkets)
    --print("Client event triggered")
    store_one = openMarkets[1]
    store_two = openMarkets[2]
    --print(store_one)
    --print(store_two)
end)
    ...
    while true do
        ...
        for storeId, storeConfig in pairs(Config.Stores) do
            if storeConfig.StoreHoursAllowed then
                if storeId == store_one or storeId == store_two then
                    --print("We are in")
...
function OpenSellMenu(storeId, category)
    ...
    Core.RpcCall('black_market:callback:getShopStock', function(result)
        if not result then
            closeAll()
            return
        end
        ...
        MenuData.Open('default', GetCurrentResourceName(), 'menuapi' .. storeId .. category, {
            ...
            if data.current.value == "sell" then
                ...
                TriggerServerEvent("black_market:Client:sellItems", SellTable, storeId)
                closeAll()
                SellTable = {}
            end
        end, function(data, menu)
...
function OpenBuyMenu(storeId, category)
    ...
    Core.RpcCall('black_market:callback:ShopStock', function(result)
        if not result then
            closeAll()
            return
        end
        ...
        MenuData.Open('default', GetCurrentResourceName(), 'menuapi' .. storeId .. category, {
            ...
            if data.current.value == "finish" then
                ...
                TriggerServerEvent("black_market:Client:buyItems", BuyTable, storeId) -- sell it
                BuyTable = {}
                closeAll()
            end
        end, function(data, menu)
...
-- GET CHARACTER SPAWN EVENT


RegisterNetEvent("black_market:RefreshStorePrices", function(SellItems, BuyItems)
...
AddEventHandler('onClientResourceStart', function(resourceName)
    ...
    Wait(1000)
    TriggerServerEvent("black_market:GetRefreshedPrices")
    --Zurv4n added
    TriggerServerEvent("black_market:GetOpenBlackMarket")
end)
...
RegisterNetEvent("black_market:Server:tableOfJobs", function(jobs)
    Jobs = jobs
    Wait(1000)
    Spawn = true
end)
